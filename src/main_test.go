...
func EqualCallTot(msg string, t *testing.T, xs []Price, f func(price Price) float64, want []float64, tol float64) {
...
func TestEuropeanCallATMStatsAreCorrect(t *testing.T) {
	...
	spots := make([]Price, 10)
	for i := 0; i < 10; i++ {
		spots[i] = Price(100.0 + float64(i-5)*15.0)
	}

	EqualCallTot(
		"payoffs",
		t, spots,
		func(price Price) float64 { return float64(opt.payoff(price)) },
	...
	EqualCallTot(
		"prices",
		t, spots,
		func(price Price) float64 { return float64(opt.price(price, 0)) },
	...
	EqualCallTot(
		"deltas",
		t, spots,
		func(price Price) float64 { return delta(opt, price, 0) },
	...
	EqualCallTot(
		"gammas",
		t, spots,
		func(price Price) float64 { return gamma(opt, price, 0) },
	...
	EqualCallTot(
		"thetas",
		t, spots,
		func(price Price) float64 { return theta(opt, price, 0) },
	...
	EqualCallTot(
		"rhos",
		t, spots,
		func(price Price) float64 { return opt.rho(price, 0) },
